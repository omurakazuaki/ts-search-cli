...
## Phase 0: Preparation & PoC (Current)
- [x] ESLint & Prettier Setup
- [ ] **Task 0.1**: Setup Testing Framework (Jest)
    - Install `jest`, `ts-jest`, `@types/jest`.
    - Configure `jest.config.js`.
- [ ] **Task 0.2**: Verify PoC
    - Run `npm start` to verify `poc.ts` interacts correctly with `typescript-language-server`.

## Phase 1: Domain & Ports (Design First)
*Goal: Define the core language of the system without implementation details.*
- [ ] **Task 1.1**: Define Domain Entities
...
- [ ] **Task 1.2**: Define UseCase Ports (Interfaces)
...
## Phase 2: UseCase Logic (Pure Business Logic)
*Goal: Implement application logic with Mock repositories. 100% Unit Test coverage.*
- [ ] **Task 2.1**: MapFileUseCase
    - Create `src/usecases/MapFileUseCase.ts`.
    - Create `src/usecases/MapFileUseCase.spec.ts` (Mock `ILspRepository`).
- [ ] **Task 2.2**: FindSymbolUseCase
    - Create `src/usecases/FindSymbolUseCase.ts`.
    - Create `src/usecases/FindSymbolUseCase.spec.ts`.
- [ ] **Task 2.3**: InspectCodeUseCase
...
*Goal: Implement the actual communication with `typescript-language-server`.*
